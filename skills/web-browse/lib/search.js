...
export async function searchSearx(httpFetch, headers, query, num) {
  const url = `https://searx.be/search?q=${encodeURIComponent(query)}&format=html&language=en`;
  const response = await httpFetch(url, { headers });
  if (!response.ok) throw new Error(`Search failed: ${response.status} ${response.statusText}`);
  ...
  $("article.result").each((i, el) => {
    ...
    const snippetEl = $(el).find("p.content, p.result-content");
...
/**
 * Main search flow: try Google (via browser context) first, then fall back.
 */
export async function searchWebFromContext({
  ...
  if (results.length === 0) {
    log("Google returned no results. Falling back to Searx...");
    try {
      results = await searchSearx(httpFetch, headers, query, numResults);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      log(`Searx search failed: ${message}`);
    }
  }

  if (results.length === 0) {
    log("Searx returned no results. Falling back to DuckDuckGo...");
    try {
